...
const addUsers = async (activeUserCount, inactiveUserCount = 0) => {
  const hash = await bcrypt.hash('P4sswrod', 10);
...
describe('Listing Users', () => {
  ...
  it('returns 1st page when page is set below zero as request parameter', async () => {
    await addUsers(11);
  ...
  it('returns user page without logged in user when request has valid authorization', async () => {
    await addUsers(11);
    const response = await getUsers({ auth: { email: 'user1@gmail.com', password: 'P4ssword' } });
